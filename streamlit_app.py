...
with coluna1:
    ...
    posicao = st.selectbox(
        ...
        help="Informação sobre o percurso observado da embarcação. Deve indicar se a trajetória parece normal ou suspeita."
    )
    velocidade = st.selectbox(
        "Velocidade/Curso",
        ["Normal", "Ligeiramente suspeito", "Muito suspeito"],
        help="Avaliação da coerência da velocidade e da direção da embarcação ao longo do tempo."
    )
    ...
    alerta = st.selectbox(
        ...
        help="Classificação qualitativa do alerta gerado pelo sistema automático. Indica se o caso parece pouco, moderadamente ou muito suspeito."
    )
    pontuacao_detetor = st.slider(
        "Pontuação do detetor",
        0, 100, 50,
        help="Valor numérico associado ao grau de suspeita identificado pelo detetor. Quanto maior a pontuação, maior a suspeita."
    )
    ...
    radar = st.selectbox(
        ...
        help="Indica se outras fontes de informação confirmam ou contradizem os dados principais observados."
    )
    contexto = st.selectbox(
        "Contexto operacional",
        ["Normal", "Pouco habitual", "Muito suspeito"],
        help="Representa se a situação observada é normal ou invulgar no contexto operacional em análise."
    )
...
with coluna2:
    ...
    i1 = st.selectbox(
        ...
        ["Baixo", "Médio", "Elevado"],
        help="Avalia se existem sinais de inconsistência na identificação da embarcação."
    )
    i2 = st.selectbox(
        "I2 - Alteração anormal de identidade",
        ["Baixo", "Médio", "Elevado"],
        help="Avalia mudanças inesperadas ou incoerentes nos elementos de identidade."
    )
    i3 = st.selectbox(
        "I3 - Plausibilidade cinemática",
        ["Baixo", "Médio", "Elevado"],
        help="Verifica se o comportamento do movimento da embarcação é fisicamente plausível."
    )
    i4 = st.selectbox(
        "I4 - Consistência espaço-temporal",
        ["Baixo", "Médio", "Elevado"],
        help="Verifica se a evolução da posição e do tempo faz sentido de forma coerente."
    )
    i5 = st.selectbox(
        "I5 - Consistência contextual",
        ["Baixo", "Médio", "Elevado"],
        help="Analisa se o comportamento observado é coerente com o contexto operacional."
    )
    i6 = st.selectbox(
        "I6 - Consistência entre fontes",
        ["Baixo", "Médio", "Elevado"],
        help="Avalia se diferentes fontes de informação apresentam uma interpretação consistente do caso."
    )
...
if st.button("Gerar recomendação"):
    ...
    with c1:
        st.metric("Pontuação total", pontuacao_total, help="Resultado agregado da avaliação dos indicadores.")
    with c2:
        st.metric("Nível de risco", risco, help="Classificação global do risco com base na pontuação obtida.")
    with c3:
        st.metric("Ação proposta", acao, help="Recomendação inicial gerada pelo sistema para apoiar a decisão humana.")
    ...
    decisao_utilizador = st.selectbox(
        ...
        help="Permite ao utilizador confirmar ou alterar a recomendação proposta pelo sistema."
    )
    ...
    justificacao = st.text_area(
        ...
        help="Campo destinado à justificação da decisão final tomada pelo utilizador."
    )
